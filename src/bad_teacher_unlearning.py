...
def validation_step(model, batch, device):
    ...
    _, preds = torch.max(out, dim=1)       # Calculate accuracy
    acc = torch.tensor(torch.sum(preds == labels.to(device)).item() / len(preds))*100   
...
def unlearning_step(model, unlearning_teacher, full_trained_teacher, unlearn_data_loader, 
                    optimizer, device='cuda', KL_temperature = 1.0):
    losses = []
...
def fit_one_unlearning_cycle(epochs, model, train_loader, val_loader, lr, device='cuda'):
    history = []
