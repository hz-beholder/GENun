...
def parse_args():
    ...
    parser.add_argument('--log_dir', default='./logs/cifar10_-1_difval', help='Folder where all logs are stored (default: .)')
    parser.add_argument('--out_dir', default='./outs/cifar10_-1_difval', help='Folder where all outputs are stored (default: .)') 
    parser.add_argument('--exp_name', default='rob_log', help='Subfolder where all logs are stored (default: rob_log)')
    ...
    ## running parameter
    parser.add_argument('--protocal', type=str, default='FT', #'DU', #
                        choices=['DU', 'FT', 'LASTK', 'RL', 'GA', 'DISTILL'])
...
def model_testing(model, data, data_retain, data_unlearn, batch_size, num_workers, 
    ...
    #     logger.info(f" +++++++++++ Forget Class: {forget_classes} +++++++++++ ")
    #     for type_, dt_ in data_unlearn.items():
    #         acc_dt, auc_dt = __data_test__(model, dt_, batch_size, num_workers, device, transform)
...
if __name__ == "__main__":
    ...
    robun.set_params(
        ...
        args.adv_lamb, args.feed_lamb, args.weight_decay, args.momentum,
    ...
    acc_org, auc_org = robun.model_evaluate(model_, test_loader, device)
    ...
    modfn = f"{args.arch}_{args.protocal}_{trans_params}_adv-{adv_losser}_fdbk-{feed_losser}_clsreg-{args.classify_lamb}_{robun._suffix_}_best.pt"   # -single
    modout_path = os.path.join(robun.path['model'], modfn)
