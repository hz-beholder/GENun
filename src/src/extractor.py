...
## Using the stable logit scaling function from LiRA paper
class LIRAFExtractor2(ExtractBase):
    """LiRA稳定对数比特征提取器"""
    
    def __init__(self, batch_size=32):
        super(LIRAFExtractor2, self).__init__(stacked=False, include_posterior=False)
        self.batch_size = batch_size
    
    def extract_feature(self, model, dataset, device='cuda'):
        """提取LiRA稳定对数比特征 φ_stable = log(f(x)_y) - log(∑_{y'≠y} f(x)_{y'})"""
        model.eval()
        model.to(device)
        ...
        with torch.no_grad():
            for inputs, labels in loader:
                ...
                other_probs_sum = torch.sum(probs * mask.float(), dim=1)
                
                # 计算稳定对数比 φ_stable = log(f(x)_y) - log(∑_{y'≠y} f(x)_{y'})
                # 添加小值避免数值不稳定
                epsilon = 1e-10
                phi_stable = torch.log(true_class_probs + epsilon) - torch.log(other_probs_sum + epsilon)
                
                features.append(phi_stable.cpu().numpy())
        
        features = np.concatenate(features, axis=0)
        return features.reshape(-1, 1)

    def _stable_logit(self, probs, labels):
        """计算稳定对数比的辅助函数"""
        batch_size = probs.size(0)
        ...
        # 计算 φ_stable = log(f(x)_y) - log(∑_{y'≠y} f(x)_{y'})
        epsilon = 1e-10
        phi_stable = torch.log(true_probs + epsilon) - torch.log(other_probs_sum + epsilon)
...
def get_extractor(attack_feature, **kwargs):
    ...
    elif attack_feature == 'lira2':
        batch_size = kwargs.get('batch_size', 32)
        return LIRAFExtractor2(batch_size)
    else:
