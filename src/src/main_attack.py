...
def parse_args():
    ...
    parser.add_argument('--attack_feature', type=str, default='loss', #'linear',  #'loss', #'gradient', #
                        choices=['entropy', 'loss', 'posterior', 'linear', 'nonlinear', 'mixlayer', 'gradient', 'gradientnorm','lira','lira2'], 
...
if __name__ == "__main__":
    ...
    if not os.path.exists(attack_data_path):
        ...
        # 只在LiRA攻击时修改transform
        if args.attack_feature == 'lira' or args.attack_model == 'lira':
            ...
            compatible_transform = preproc.get_transform("autoaug", num_classes)
