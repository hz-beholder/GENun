...
# Pruning operation
def pruning_model(model, prune_amount):
    ...
    params_to_prune = tuple(params_to_prune)
...
def pruning_model_structured(model, prune_amount):
    # print("Apply Unstructured L1 Pruning Globally (all conv layers)")
    for name, m in model.named_modules():
        if isinstance(m, nn.Conv2d):
            prune.ln_structured(
...
def pruning_model_structured_channel_wise(model, prune_amount):
...
def count_sparsity(model):
    ...
    # print("Sparsity in total:", sparsity)
    return zero_count
...
def fetch_data(data_loader, num_classes, samples_per_class):
...
def snip_importance_score(model, data_loader, samples_per_class, loss_func=torch.nn.CrossEntropyLoss()):
    score_dict = {}
    model.zero_grad()
    device = next(model.parameters()).device
    x, y = fetch_data(data_loader, model.fc.out_features, samples_per_class)
...
def grasp_importance_score(model, data_loader, samples_per_class, loss_func=torch.nn.CrossEntropyLoss()):
    score_dict = {}
    model.zero_grad()
    device = next(model.parameters()).device
    x, y = fetch_data(data_loader, model.fc.out_features, samples_per_class)
...
def synflow_importance_score(model, data_loader):
    @torch.no_grad()
    def linearize(model):
        signs = {}
...
def global_prune_model(model, ratio, method, data_loader=None, structured=False, sample_per_classes=25):
    if method == "mp":
        score_dict = mp_importance_score(model)
    elif method == "snip":
        score_dict = snip_importance_score(model, data_loader, sample_per_classes)
    elif method == "grasp":
        score_dict = grasp_importance_score(model, data_loader, sample_per_classes)
    elif method == "synflow":
        pass
    else:
    ...
    if method == "synflow":
        ...
        for _ in range(iteration_number):
            score_dict = synflow_importance_score(model, data_loader)
            if structured:
                pass
            else:
                prune.global_unstructured(
