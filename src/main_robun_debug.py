...
def parse_args():
    parser = argparse.ArgumentParser(description='Robust Unlearning Model Under Feedback')
    parser.add_argument('--logname', default='debug', help='name of output log file')
    parser.add_argument('--log_dir', default='.', help='Folder where all logs are stored (default: .)')
    parser.add_argument('--out_dir', default='./outs/cifar10_-1', help='Folder where all outputs are stored (default: .)')
    parser.add_argument('--exp_name', default='rob_log_debug', help='Subfolder where all logs are stored (default: logs)')
    ...
    parser.add_argument('--num_to_forget', type=int, default=0000, metavar='Nf', help='number of samples to forget (default: 5000)')
    parser.add_argument('--forget_classes', type=list_of_ints, default=0, metavar='Cf', help='classes to forget (default: None)')
    
    parser.add_argument('--arch', type=str, default='resnet18',  #'simple_cnn',  #
    ...
    parser.add_argument('--epochs', type=int, default=30, metavar='E', help='number of epochs for unlearning (default: 20)')
    ...
    parser.add_argument('--strength', type=float, default=2.0, help='strength of the feedback (default: 1.0)')
    
    ## running parameter
    parser.add_argument('--protocal', type=str, default='FT', #'DISTILL', #'LASTK', #
    ...
    parser.add_argument('--adv_loss', type=str, default='udp', #'none', # 'fgsm', #'vat', #
    ...
    parser.add_argument('--feedback', type=str, default= "snn", #'none', #'entropy', #'ot', #'contrast', #
                        choices=['none', 'contrast', 'snn', 'mmd', 'ot', 'entropy', 'max_entropy', 'kl', 'mse', 'l2',],
...
def get_feedback_loss(args, device):
    ...
    if option == 'none':
        ...
        # print("Not a valid triplet loss")
        return option
...
if __name__ == "__main__":
    ...
    # args.model_path = './outs_debug/cifar10/model_bases/ORG0/' + mod_fn
    ...
    args.model_path="./outs/cifar10_-1_FGCLS-0/model_bases/cifar10_resnet18/ORG/cifar10_resnet18_seed-0_Nf-10000_Cf-\[0\]_ep-40_bs-256_lr-\[0_0001-0_001\]_wd-5e-05_opt-Adam_trTrans-normal_tsTrans-test_regular_0.0_sched-None.pth"
    ...
    logger.info("Initial Adversarial and Feedback loss")
    ...
    logger.info(" ###### [Train, Valid, Test, Forget, Remain-Data] "
                      ...
                      # + f"  Acc: {train_acc:.3f}, {valid_acc:.3f}, {test_acc:.3f}, {forget_acc:.3f} ######")
    ...
    ## apply different unlearn models based on the protocal
    unlearner = None
    ...
    robun = unlearner(logpath, args.logname, args.out_dir, out_name='model_ft_debug')
    robun.logger.info(args)
    logger.info(f" +++++++  Selected top-K informative samples: {args.top_k}")
    
    # model setting
    logger.info("Set parameters for unlearning")
    robun.set_params(
    ...
    if unlearner == RobustUnlearn:   # alignment-based unlearning
    ...
    for type_ , loader in data.items():
        acc_dt, auc_dt = data_model_test(unlearn_mod, loader, args.batch_size, args.num_workers, device, transform)
        logger.info(f"##### [ {type_:6} ]: ACC: {acc_dt:.4f}")   # , AUC: {auc_dt:.4f}
    ...
    if args.forget_classes is None:
        ...
        logger.info(
            ...
            + f"  Acc: {train_acc:.4f}, {train_retain_acc:.4f}, {valid_acc:.4f}, {test_acc:.4f}, {forget_acc:.4f} ######"
