...
class TinyImageNet_finetune5(VisionDataset):
    ...
    def __init__(self, root, select_class=[0,1,2,3,4], num_samples=150, train=True, transform=None, target_transform=None):
        ...
        for i in select_class:
            if self.train:
                sub_cls_id = np.random.choice(np.where(targets==i)[0], num_samples, replace=False)
            else:
                sub_cls_id = np.random.choice(np.where(targets==i)[0], int(num_samples * 0.8), replace=False)
