...
class Small_CIFAR10(VisionDataset):
    def __init__(self, root, num_samples=200, train=True, transform=None, target_transform=None):
        ...
        for i in np.unique(self.targets):
            if self.train:
                sub_cls_id = np.random.choice(np.where(ds.targets==i)[0], num_samples, replace=False)
            else:
                sub_cls_id = np.random.choice(np.where(ds.targets==i)[0], num_samples, replace=False)
...
class Small_CIFAR5(VisionDataset):
    def __init__(self, root, select_classes=[0,1,2,3,4], num_samples=150, train=True, transform=None, target_transform=None):
        ...
        for i in select_classes:
            if self.train:
                ...
                sub_cls_id = np.random.choice(np.where(ds.targets == i)[0], int(num_samples * 0.8), replace=False)
                #np.where(ds.targets==i)[0]                
            sub_ds_data_list.append(ds.data[sub_cls_id,:,:,:])
            sub_ds_target_list.append(ds.targets[sub_cls_id])
        self.data=np.concatenate(sub_ds_data_list)
...
class Small_Binary_CIFAR10(VisionDataset):
    def __init__(
        ...
        super(Small_Binary_CIFAR10, self).__init__(root, transform=transform, target_transform=target_transform)
        self.train = train
        ds = torchvision.datasets.CIFAR10(root=root, train=True, download=True)
        ds.targets = np.array(ds.targets)
        sub_ds_data_list = []
